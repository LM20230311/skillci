...
function validatePolicy(policyPath) {
  const path2 = (0, import_node_path.resolve)(policyPath);
  const diagnostics = [];
  ...
  let section;
  let list;
  for (const [offset, rawLine] of (0, import_node_fs.readFileSync)(path2, "utf8").split(/\r?\n/).entries()) {
    if (/^\s*#/.test(rawLine))
      continue;
...
function renderPolicyDiffGitHub(diff) {
  const file = (0, import_node_path.relative)(process.cwd(), diff.after.path) || diff.after.path;
...
function audit(targetPath, options = {}) {
  const absoluteTarget = (0, import_node_path2.resolve)(targetPath);
  if (!(0, import_node_fs2.existsSync)(absoluteTarget)) {
  ...
  for (const file of files) {
    const content = (0, import_node_fs2.readFileSync)(file, "utf8");
    const lines = content.split(/\r?\n/);
    let nextLineSuppression;
    lines.forEach((line, index) => {
      const relativeFile = (0, import_node_path2.relative)(absoluteTarget, file) || file;
...
function collectFiles(target) {
  const targetStats = (0, import_node_fs2.statSync)(target);
  if (targetStats.isFile())
    return isTextFile(target) ? [target] : [];
  const files = [];
  for (const entry of (0, import_node_fs2.readdirSync)(target, { withFileTypes: true })) {
    if (entry.isDirectory()) {
      if (!SKIPPED_DIRECTORIES.has(entry.name))
        files.push(...collectFiles((0, import_node_path2.resolve)(target, entry.name)));
    } else if (entry.isFile() && isTextFile(entry.name)) {
      files.push((0, import_node_path2.resolve)(target, entry.name));
    }
  }
  return files;
}
...
function validateBehaviorCase(casePath) {
  const path2 = (0, import_node_path3.resolve)(casePath);
  const diagnostics = [];
  if (!(0, import_node_fs3.existsSync)(path2))
    return invalid(path2, "Case file does not exist.");
  let raw;
  try {
    raw = (0, import_yaml.parse)((0, import_node_fs3.readFileSync)(path2, "utf8"));
  } catch (error2) {
    return invalid(path2, `Invalid YAML: ${error2 instanceof Error ? error2.message : String(error2)}`);
  }
  if (!isObject(raw))
    return invalid(path2, "A behavior case must be a YAML object.");
  ...
  if (fixture && isSafeRelativePath(fixture) && !(0, import_node_fs3.existsSync)((0, import_node_path3.resolve)((0, import_node_path3.dirname)(path2), fixture)))
  ...
  const allowedTools = tools ? stringArray(tools.allow, "tools.allow", diagnostics) : void 0;
  const deniedTools = tools ? stringArray(tools.deny, "tools.deny", diagnostics) : void 0;
  ...
  const files = expect ? objectValue(expect.files, "expect.files", diagnostics) : void 0;
  if (files)
    checkKeys(files, ["created", "modified", "unchanged"], "expect.files", diagnostics);
  const created = files ? stringArray(files.created, "expect.files.created", diagnostics) : void 0;
  const modified = files ? stringArray(files.modified, "expect.files.modified", diagnostics) : void 0;
  const unchanged = files ? stringArray(files.unchanged, "expect.files.unchanged", diagnostics) : void 0;
...
function invalid(path2, message) {
  return { path: path2, diagnostics: [{ path: "case", message }], valid: false };
}
function isObject(value) {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}
function objectValue(value, path2, diagnostics) {
  if (!isObject(value)) {
    diagnostics.push({ path: path2, message: "Expected an object." });
    return void 0;
  }
  return value;
}
function stringValue(value, path2, diagnostics) {
  if (typeof value !== "string" || !value.trim()) {
    diagnostics.push({ path: path2, message: "Expected a non-empty string." });
    return void 0;
  }
  return value;
}
function numberValue(value, path2, diagnostics) {
  if (typeof value !== "number") {
    diagnostics.push({ path: path2, message: "Expected a number." });
    return void 0;
  }
  return value;
}
function stringArray(value, path2, diagnostics) {
  if (!Array.isArray(value) || value.some((item) => typeof item !== "string" || !item.trim())) {
    diagnostics.push({ path: path2, message: "Expected a list of non-empty strings." });
    return void 0;
  }
  if (new Set(value).size !== value.length)
    diagnostics.push({ path: path2, message: "List values must be unique." });
  return value;
}
function checkKeys(value, allowedKeys, path2, diagnostics) {
  for (const key of Object.keys(value))
    if (!allowedKeys.includes(key))
      diagnostics.push({ path: path2, message: `Unknown field: ${key}.` });
}
...
function runCases(casesPath) {
  const absoluteCasesPath = (0, import_node_path4.resolve)(casesPath);
...
function runCase(caseFile) {
  let definition;
  try {
    definition = parseCase((0, import_node_fs4.readFileSync)(caseFile, "utf8"));
    const base = (0, import_node_path4.dirname)(caseFile);
    const result = audit((0, import_node_path4.resolve)(base, definition.target), { policyPath: definition.policy ? (0, import_node_path4.resolve)(base, definition.policy) : void 0 });
    ...
    return {
      name: definition?.name ?? (0, import_node_path4.relative)(process.cwd(), caseFile),
      caseFile,
      passed: false,
...
function collectCaseFiles(path2) {
  const stats = (0, import_node_fs4.statSync)(path2);
  if (stats.isFile())
    return isCaseFile(path2) ? [path2] : [];
  return (0, import_node_fs4.readdirSync)(path2, { withFileTypes: true }).flatMap((entry) => {
    const child = (0, import_node_path4.resolve)(path2, entry.name);
    if (entry.isDirectory())
...
function initialize(targetPath = ".") {
  const root = (0, import_node_path5.resolve)(targetPath, "skillci");
  const cases = (0, import_node_path5.resolve)(root, "cases");
  const fixtures = (0, import_node_path5.resolve)(root, "fixtures", "docs-skill");
  (0, import_node_fs5.mkdirSync)(cases, { recursive: true });
  (0, import_node_fs5.mkdirSync)(fixtures, { recursive: true });
  const created = [];
  for (const [path2, content] of [[(0, import_node_path5.resolve)(root, "policy.yml"), POLICY], [(0, import_node_path5.resolve)(cases, "smoke.yml"), CASE], [(0, import_node_path5.resolve)(fixtures, "SKILL.md"), SAMPLE_SKILL]]) {
    if ((0, import_node_fs5.existsSync)(path2))
      continue;
    (0, import_node_fs5.writeFileSync)(path2, content, "utf8");
    created.push(path2);
  }
  return created;
}
...
` : format === "github" ? renderGitHubAnnotations(result) : renderMarkdown(result);
  ...
  if (outputPath) {
    (0, import_node_fs6.writeFileSync)(outputPath, output, "utf8");
