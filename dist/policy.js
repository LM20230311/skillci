...
export function loadPolicy(policyPath) {
    ...
    if (!existsSync(absolutePath))
        throw new Error(`Policy does not exist: ${policyPath}`);
    const policy = { path: absolutePath, deniedNetwork: false, deniedPaths: [], deniedCommands: [] };
    let inDeny = false;
    let list;
    for (const rawLine of readFileSync(absolutePath, "utf8").split(/\r?\n/)) {
        ...
        if (value && list) {
            ...
            if (list === "paths")
                policy.deniedPaths.push(normalized);
            else
                policy.deniedCommands.push(normalized);
        }
    }
    return policy;
}
